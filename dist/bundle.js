...
exports.BUTTONS = exports.SOUNDS = exports.TIMING = exports.TEXT = exports.FINAL_STAGE = undefined;
 ...
 * @type {Number}
 */
var FINAL_STAGE = exports.FINAL_STAGE = 20;
...
exports.wrongState = exports.advanceState = exports.testButtonPress = exports.formatStageNum = exports.objectKeyByValue = exports.iterObj = exports.getButtonsWithinCurrent = exports.setStrict = exports.getButtonSeries = exports.getDurations = undefined;

var _constants = __webpack_require__(49);

function _toConsumableArray(arr) { if (Array.isArray(arr)) { for (var i = 0, arr2 = Array(arr.length); i < arr.length; i++) { arr2[i] = arr[i]; } return arr2; } else { return Array.from(arr); } }

var newMappedArray = function newMappedArray(n, fn) {
  return [].concat(_toConsumableArray(Array(n))).map(fn);
};

var getNewState = function getNewState(oldState, stateDelta) {
  return Object.assign({}, oldState, stateDelta);
};

var getTimeout = function getTimeout(n, tv) {
  return function (_, i) {
    return i < n * (1 / 5) ? tv[0] : i < n * (2 / 5) ? tv[1] : i < n * (3 / 5) ? tv[2] : tv[3];
  };
};

var getRandomButton = function getRandomButton(buttons) {
  return function () {
    return buttons[Math.floor(Math.random() / (1 / buttons.length))];
  };
};

var isWithin = function isWithin(limit) {
  return function (_, i) {
    return i < limit;
  };
};
 ...
 */
var getButtonSeries = exports.getButtonSeries = function getButtonSeries(n, buttons) {
  return newMappedArray(n, getRandomButton(buttons));
};
 ...
 */
var setStrict = exports.setStrict = function setStrict(oldState, isStrict) {
  return getNewState(oldState, { isStrict: isStrict });
};
 ...
 */
var getButtonsWithinCurrent = exports.getButtonsWithinCurrent = function getButtonsWithinCurrent(state) {
  return state.buttonSeries.filter(isWithin(state.currentStage));
};
 ...
 * @param  {Function} fn
 */
var iterObj = exports.iterObj = function iterObj(obj, fn) {
  return Object.values(obj).forEach(fn);
};
 ...
 * @return {string}
 */
var objectKeyByValue = exports.objectKeyByValue = function objectKeyByValue(obj, val) {
 ...
 */
var formatStageNum = exports.formatStageNum = function formatStageNum(n) {
 ...
 * @return {object}
 */
var testButtonPress = exports.testButtonPress = function testButtonPress(id, oldState) {
  return getNewState(oldState, {
    isCorrect: objectKeyByValue(_constants.BUTTONS, id) === oldState.buttonSeries[oldState.toTest]
  });
};
 ...
 * @return {object}
 */
var advanceState = exports.advanceState = function advanceState(oldState) {
  return getNewState(oldState, oldState.toTest + 1 === oldState.currentStage ? {
...
exports.wrongButtonPressed = wrongButtonPressed;

__webpack_require__(127);

var _initialState = __webpack_require__(332);
...
var _domManipulation = __webpack_require__(333);
...
var strictSwitch = document.querySelector('#myonoffswitch');
...
function handleSimonButtonsUp(event) {
  ...
  (0, _domManipulation.buttonUnpressed)(event.target.id);
  state = (0, _utils.testButtonPress)(event.target.id, state);
  if (state.isCorrect) correctButtonPressed();else wrongButtonPressed();
}
...
var simonButtons = document.querySelector('#simon-buttons');
 ...
 */
var buttonSeries = function buttonSeries() {
  return (0, _utils.getButtonSeries)(_constants.FINAL_STAGE, Object.keys(_constants.BUTTONS));
};
...
var _sound = __webpack_require__(334);
...
var addClassTo = function addClassTo(id, cls) {
  return element(id).classList.add(cls);
};

var removeClassFrom = function removeClassFrom(id, cls) {
  return element(id).classList.remove(cls);
};

var makeClickable = function makeClickable(id) {
  return removeClassFrom(id, 'unclickable');
};

var makeButtonsClickable = function makeButtonsClickable() {
  return (0, _utils.iterObj)(_constants.BUTTONS, makeClickable);
};

var makeUnclickable = function makeUnclickable(id) {
  return addClassTo(id, 'unclickable');
};

var makeButtonsUnclickable = function makeButtonsUnclickable() {
...
var isStopped = exports.isStopped = function isStopped() {
  return element('btn-start').innerText === _constants.TEXT.startButtonStopped;
};
...
var playButton = function playButton(button, buttons, duration) {
  ...
  makeLit(buttonID);
  var sound = (0, _sound2.default)(button);
  setTimeout(function () {
    makeUnlit(buttonID);
    sound.stop();
    setTimeout(function () {
...
var buttonPressedSound = void 0;

var buttonPressed = exports.buttonPressed = function buttonPressed(id) {
  makeLit(id);
  buttonPressedSound = (0, _sound2.default)((0, _utils.objectKeyByValue)(_constants.BUTTONS, id));
};
...
var showStage = exports.showStage = function showStage(state) {
  showStageMsg(_constants.TEXT.stagePrefix + (0, _utils.formatStageNum)(state.currentStage));
};

var gameStarted = exports.gameStarted = function gameStarted(state) {
  showStage(state);
  showStartMsg(_constants.TEXT.startButtonStarted);
  playButtonSeries(state);
};

var gameStopped = exports.gameStopped = function gameStopped() {
  makeButtonsUnclickable();
  showStageMsg(_constants.TEXT.stagePrefix + _constants.TEXT.stageEmpty);
  showStartMsg(_constants.TEXT.startButtonStopped);
};
...
var wrongDom = exports.wrongDom = function wrongDom(state) {
  makeButtonsUnclickable();
  showStageMsg(_constants.TEXT.stageWrong);
  var sound = (0, _sound2.default)('wrong');
  makeLit(_constants.BUTTONS[state.lastWrong]);
  setTimeout(function () {
    sound.stop();
    makeUnlit(_constants.BUTTONS[state.lastWrong]);
    setTimeout(function () {
...
var gameWon = exports.gameWon = function gameWon() {
  makeButtonsUnclickable();
  setTimeout(function () {
    showStageMsg(_constants.TEXT.stageWinning);
    var sound = (0, _sound2.default)('win');
    makeButtonsLit();
    setTimeout(function () {
      showStageMsg(_constants.TEXT.stagePrefix + _constants.TEXT.stageEmpty);
      showStartMsg(_constants.TEXT.startButtonStopped);
